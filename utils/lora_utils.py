...
def train_lora(image0, image1, prompt, model_path, vae_path, save_lora_path, lora_step, lora_lr, lora_rank, progress):
    ...
    for _ in progress.tqdm(range(lora_step), desc="training LoRA"):
        unet.train()
        image_transformed = []
        image_transformed.append(image_transforms(image0).to(device, dtype=torch.float16).unsqueeze(0))
        image_transformed.append(image_transforms(image1).to(device, dtype=torch.float16).unsqueeze(0))
        image_transformed = torch.cat(image_transformed,dim=0)
        ...
        model_pred = unet(noisy_model_input, timesteps, text_embedding.repeat(2,1,1)).sample
