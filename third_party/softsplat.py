...
def softsplat(tenIn:torch.Tensor, tenFlow:torch.Tensor, tenMetric:torch.Tensor, strMode:str):
    assert(strMode.split('-')[0] in ['sum', 'avg', 'linear', 'soft', 'max'])
    ...
    elif strMode.split('-')[0] == 'max':
        tenIn = torch.cat([tenIn, tenMetric], 1)
    # end

    tenOut, Sq = softsplat_func.apply(tenIn, tenFlow, True)
    if strMode.split('-')[0] == 'max':
        new_indices = Sq[:,1:].permute(0,2,3,1)
        mask = torch.ones(1,1,tenIn.shape[2],tenIn.shape[3]).cuda()
        new_indices = normalize_for_grid_sample(new_indices, new_indices.shape[1], new_indices.shape[2])
        tenOut[:,:-1] = F.grid_sample(tenIn[:,:-1], new_indices, mode='nearest',padding_mode='border')
        mask  = F.grid_sample(mask, new_indices, mode='bilinear',padding_mode='zeros')
        tenOut[:,-1] = Sq[:,0]
        return tenOut, mask
...
class softsplat_func(torch.autograd.Function):
    ...
    def forward(self, tenIn, tenFlow, flag):
        tenOut = tenIn.new_zeros([tenIn.shape[0], tenIn.shape[1], tenIn.shape[2], tenIn.shape[3]])
        tenSquare = tenIn.new_zeros([tenIn.shape[0], 3, tenIn.shape[2], tenIn.shape[3]])
            ...
            cuda_launch(cuda_kernel('softsplat_out', '''
                extern "C" __global__ void __launch_bounds__(512) softsplat_out(
                    ...
                    if ((intNorthwestX >= 0) && (intNorthwestX < SIZE_3(tenOut)) && (intNorthwestY >= 0) && (intNorthwestY < SIZE_2(tenOut))) {
                        ...
                        if ((VALUE_4(tenSquare, intN, 0, intNorthwestY, intNorthwestX) < fltIn * fltNorthwest) && (intC == 4)){
                            atomicExch(&tenSquare[OFFSET_4(tenOut, intN, 0, intNorthwestY, intNorthwestX)], fltIn);
                            atomicExch(&tenSquare[OFFSET_4(tenOut, intN, 1, intNorthwestY, intNorthwestX)], intX);
                            atomicExch(&tenSquare[OFFSET_4(tenOut, intN, 2, intNorthwestY, intNorthwestX)], intY);}
                    }
                    ...
                    if ((intNortheastX >= 0) && (intNortheastX < SIZE_3(tenOut)) && (intNortheastY >= 0) && (intNortheastY < SIZE_2(tenOut))) {
                        ...
                        if ((VALUE_4(tenSquare, intN, 0, intNortheastY, intNortheastX) < fltIn * fltNortheast) && (intC == 4)){
                            atomicExch(&tenSquare[OFFSET_4(tenOut, intN, 0, intNortheastY, intNortheastX)], fltIn);
                            atomicExch(&tenSquare[OFFSET_4(tenOut, intN, 1, intNortheastY, intNortheastX)], intX);
                            atomicExch(&tenSquare[OFFSET_4(tenOut, intN, 2, intNortheastY, intNortheastX)], intY);}
                    }
                    ...
                    if ((intSouthwestX >= 0) && (intSouthwestX < SIZE_3(tenOut)) && (intSouthwestY >= 0) && (intSouthwestY < SIZE_2(tenOut))) {
                        ...
                        if ((VALUE_4(tenSquare, intN, 0, intSouthwestY, intSouthwestX) < fltIn * fltSouthwest) && (intC == 4)){
                            atomicExch(&tenSquare[OFFSET_4(tenOut, intN, 0, intSouthwestY, intSouthwestX)], fltIn);
                            atomicExch(&tenSquare[OFFSET_4(tenOut, intN, 1, intSouthwestY, intSouthwestX)], intX);
                            atomicExch(&tenSquare[OFFSET_4(tenOut, intN, 2, intSouthwestY, intSouthwestX)], intY);}
                    }
                    ...
                    if ((intSoutheastX >= 0) && (intSoutheastX < SIZE_3(tenOut)) && (intSoutheastY >= 0) && (intSoutheastY < SIZE_2(tenOut))) {
                        ...
                        if ((VALUE_4(tenSquare, intN, 0, intSoutheastY, intSoutheastX) < fltIn * fltSoutheast) && (intC == 4)){
                            atomicExch(&tenSquare[OFFSET_4(tenOut, intN, 0, intSoutheastY, intSoutheastX)], fltIn);
                            atomicExch(&tenSquare[OFFSET_4(tenOut, intN, 1, intSoutheastY, intSoutheastX)], intX);
                            atomicExch(&tenSquare[OFFSET_4(tenOut, intN, 2, intSoutheastY, intSoutheastX)], intY);}
                    }
                } }
            ''', {
                'tenIn': tenIn,
                ...
                block=tuple([512, 1, 1]),
                args=[cuda_int32(tenOut.nelement()), tenIn.data_ptr(), tenFlow.data_ptr(), tenOut.data_ptr(), tenSquare.data_ptr()],
        ...
        if flag: return tenOut, tenSquare
        else: return tenOut
    # end
